...
<section class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h4>Requisition Request Add Items Form</h4>
            </div>
            <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                    <li class="breadcrumb-item"><a href="#">Home</a></li>
                    <li class="breadcrumb-item active">Requisition Request Add Items Form</li>
                </ol>
            </div>
        </div>
    </div>
...
<section class="content">
    <div class="card card-primary">
        <div class="col-md-12">
            <table class="table table-sm table-responsive table-sm table-responsive table-striped">
                <thead class="thead-infor">
                    <tr>
                        <th colspan="5">{{ $transi->customer->fullname }}</th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
           ...
           <div class="callout callout-info">
                <table class="table table-sm table-responsive table-bordered table-striped" width="40">
                    ...
                    <tbody>
                        @foreach($list as $requstProduct)
                        <tr>
                            <td>{{$requstProduct->product->product_name}}</td>
                            <td>{{$requstProduct->quantity }}</td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
        ...
        <div class="card-body">
            ...
            {!! Form::open(array('route' => 'saveAddItems', 'method'=>'POST', 'role'=>'form' )) !!}
            {{ csrf_field() }}
            ...
            <div class="form-group row">
                ...
                <input type="hidden" name="requisition_id" id="requisition_id" value="{{$transi->id}}" readonly="1" required="true">
            </div>
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $('select[name="product"]').on('change', function() {
            var id = $(this).val();
            var k = parseInt($("#requisition_id").val());
              $("#current_quantity").val("");
              $("#quantity").val(0);
              $("#max_discount").val("");
              $("#price").val("");
            if(id) {
                    $.ajax({
                        url: '{{ url("/myform/ajaxpriceAdd") }}/'+id+ '/' + k,
                        type: "GET",
                        ...
                        success:function(response)
                        {
                            ...
                            // alert(data.msg);
                            if(data.msg == "Record Found"){
                                Swal.fire({
                                    icon: "error",
                                    text: "The Product " + $("#product option:selected").text() + " already exist in the list of request.",
                                    showConfirmButton: true,
                                    timer: 30000
                                });
                            }
